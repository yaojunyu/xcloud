...
func printWorkflow(wf *wfv1.Xflow, outFmt string) {
	switch outFmt {
	case "name":
...
func printWorkflowHelper(wf *wfv1.Xflow, outFmt string) {
	const fmtStr = "%-20s %v\n"
...
type nodeInfoInterface interface {
	getID() string
	getNodeStatus(wf *wfv1.Xflow) wfv1.NodeStatus
	getStartTime(wf *wfv1.Xflow) metav1.Time
}
...
func (n *nodeInfo) getNodeStatus(wf *wfv1.Xflow) wfv1.NodeStatus {
	return wf.Status.Nodes[n.id]
}

func (n *nodeInfo) getStartTime(wf *wfv1.Xflow) metav1.Time {
...
type renderNode interface {
	...
	renderNodes(w *tabwriter.Writer, wf *wfv1.Xflow, depth int, nodePrefix string, childPrefix string, outFmt string)
	nodeInfoInterface
}
...
func insertSorted(wf *wfv1.Xflow, sortedArray []renderNode, item renderNode) []renderNode {
...
func attachToParent(wf *wfv1.Xflow, n renderNode,
...
func convertToRenderTrees(wf *wfv1.Xflow) map[string]renderNode {
...
func renderChild(w *tabwriter.Writer, wf *wfv1.Xflow, nInfo renderNode, depth int,
...
func printNode(w *tabwriter.Writer, wf *wfv1.Xflow, node wfv1.NodeStatus, depth int, nodePrefix string, childPrefix string, outFmt string) {
...
// boundaryNode
func (nodeInfo *boundaryNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Xflow, depth int, nodePrefix string, childPrefix string, outFmt string) {
...
func (nodeInfo *nonBoundaryParentNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Xflow, depth int, nodePrefix string, childPrefix string, outFmt string) {
	filtered, childIndent := filterNode(nodeInfo.getNodeStatus(wf))
	if !filtered {
		printNode(w, wf, nodeInfo.getNodeStatus(wf), depth, nodePrefix, childPrefix, outFmt)
	}
...
// executionNode
func (nodeInfo *executionNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Xflow, depth int, nodePrefix string, childPrefix string, outFmt string) {
