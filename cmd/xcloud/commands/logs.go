...
import (
	...
	"github.com/xcloudnative/xcloud/pkg/apis/xcloudnative.io/v1alpha1"
	wfclientset "github.com/xcloudnative/xcloud/pkg/client/clientset/versioned"
	wfinformers "github.com/xcloudnative/xcloud/pkg/client/informers/externalversions"
	"k8s.io/api/core/v1"
...
func (p *logPrinter) printRecentWorkflowLogs(wf *v1alpha1.Xflow) map[string]*time.Time {
...
func (p *logPrinter) setupWorkflowInformer(namespace string, name string, callback func(wf *v1alpha1.Xflow, done bool)) cache.SharedIndexInformer {
	...
	informer := wfInformerFactory.Xcloudnative().V1alpha1().Xflows().Informer()
	informer.AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			UpdateFunc: func(old, new interface{}) {
				updatedWf := new.(*v1alpha1.Xflow)
				if updatedWf.Name == name {
					callback(updatedWf, updatedWf.Status.Phase != v1alpha1.NodeRunning)
				}
			},
			DeleteFunc: func(obj interface{}) {
				deletedWf := obj.(*v1alpha1.Xflow)
...
func (p *logPrinter) printLiveWorkflowLogs(workflow *v1alpha1.Xflow, timeByPod map[string]*time.Time) {
	logs := make(chan logEntry)
	streamedPods := make(map[string]bool)

	processPods := func(wf *v1alpha1.Xflow) {
	...
	informer := p.setupWorkflowInformer(workflow.Namespace, workflow.Name, func(wf *v1alpha1.Xflow, done bool) {
		if done {
			close(logs)
		} else {
			processPods(wf)
