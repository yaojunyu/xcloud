...
func (woc *wfOperationCtx) executeStepGroup(stepGroup []wfv1.XflowStep, sgNodeName string, stepsCtx *stepsContext) *wfv1.NodeStatus {
...
func (woc *wfOperationCtx) resolveReferences(stepGroup []wfv1.XflowStep, scope *wfScope) ([]wfv1.XflowStep, error) {
	newStepGroup := make([]wfv1.XflowStep, len(stepGroup))
...
func (woc *wfOperationCtx) expandStepGroup(stepGroup []wfv1.XflowStep) ([]wfv1.XflowStep, error) {
	newStepGroup := make([]wfv1.XflowStep, 0)
	for _, step := range stepGroup {
...
func (woc *wfOperationCtx) expandStep(step wfv1.XflowStep) ([]wfv1.XflowStep, error) {
	stepBytes, err := json.Marshal(step)
	if err != nil {
		return nil, errors.InternalWrapError(err)
	}
	fstTmpl := fasttemplate.New(string(stepBytes), "{{", "}}")
	expandedStep := make([]wfv1.XflowStep, 0)
	var items []wfv1.Item
