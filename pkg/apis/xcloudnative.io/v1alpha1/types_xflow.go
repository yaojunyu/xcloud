...
// Xflow is the definition of a Xflow resource
// +genclient
// +genclient:noStatus
...
type Xflow struct {
	...
	Spec              XflowSpec   `json:"spec"`
	Status            XflowStatus `json:"status"`
}

// XflowList is list of Xflow resources
// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object
type XflowList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata"`
	Items           []Xflow `json:"items"`
}

// XflowSpec is the specification of a Xflow.
type XflowSpec struct {
	// Templates is a list of Xflow templates used in a Xflow
	Templates []Template `json:"templates"`

	// Entrypoint is a template reference to the starting point of the Xflow
	Entrypoint string `json:"entrypoint"`

	// Arguments contain the parameters and artifacts sent to the Xflow entrypoint
	// Parameters are referencable globally using the 'Xflow' variable prefix.
	// e.g. {{Xflow.parameters.myparam}}
	Arguments Arguments `json:"arguments,omitempty"`

	// ServiceAccountName is the name of the ServiceAccount to run all pods of the Xflow as.
	ServiceAccountName string `json:"serviceAccountName,omitempty"`

	// Volumes is a list of volumes that can be mounted by containers in a Xflow.
	Volumes []apiv1.Volume `json:"volumes,omitempty"`

	// VolumeClaimTemplates is a list of claims that containers are allowed to reference.
	// The Xflow controller will create the claims at the beginning of the Xflow
	// and delete the claims upon completion of the Xflow
	VolumeClaimTemplates []apiv1.PersistentVolumeClaim `json:"volumeClaimTemplates,omitempty"`

	// Parallelism limits the max total parallel pods that can execute at the same time in a Xflow
	Parallelism *int64 `json:"parallelism,omitempty"`

	// Suspend will suspend the Xflow and prevent execution of any future steps in the Xflow
	Suspend *bool `json:"suspend,omitempty"`

	// NodeSelector is a selector which will result in all pods of the Xflow
	// to be scheduled on the selected node(s). This is able to be overridden by
	// a nodeSelector specified in the template.
	NodeSelector map[string]string `json:"nodeSelector,omitempty"`

	// Affinity sets the scheduling constraints for all pods in the Xflow.
	// Can be overridden by an affinity specified in the template
	Affinity *apiv1.Affinity `json:"affinity,omitempty"`

	// Tolerations to apply to Xflow pods.
	...
	// Xflow, irrespective of the success, failure, or error of the
	// primary Xflow.
	OnExit string `json:"onExit,omitempty"`

	// TTLSecondsAfterFinished limits the lifetime of a Xflow that has finished execution
	// (Succeeded, Failed, Error). If this field is set, once the Xflow finishes, it will be
	...
	// ttlSecondsAfterFinished expires immediately after the Xflow finishes.
	TTLSecondsAfterFinished *int32 `json:"ttlSecondsAfterFinished,omitempty"`

	// Optional duration in seconds relative to the Xflow start time which the Xflow is
	// allowed to run before the controller terminates the Xflow. A value of zero is used to
	// terminate a Running Xflow
	ActiveDeadlineSeconds *int64 `json:"activeDeadlineSeconds,omitempty"`
	// Priority is used if controller is configured to process limited number of Xflows in parallel. Xflows with higher priority are processed first.
	Priority *int32 `json:"priority,omitempty"`
}

// Template is a reusable and composable unit of execution in a Xflow
type Template struct {
	...
	// NodeSelector is a selector to schedule this step of the Xflow to be
	// run on the selected node(s). Overrides the selector set at the Xflow level.
	NodeSelector map[string]string `json:"nodeSelector,omitempty"`

	// Affinity sets the pod's scheduling constraints
	// Overrides the affinity set at the Xflow level (if any)
	Affinity *apiv1.Affinity `json:"affinity,omitempty"`

	// Metdata sets the pods's metadata, i.e. annotations and labels
	Metadata Metadata `json:"metadata,omitempty"`

	// Deamon will allow a Xflow to proceed to the next step so long as the container reaches readiness
	Daemon *bool `json:"daemon,omitempty"`

	// Steps define a series of sequential/parallel Xflow steps
	Steps [][]XflowStep `json:"steps,omitempty"`
	...
	// Suspend template subtype which can suspend a Xflow when reaching the step
...
type Parameter struct {
	...
	// '{{Xflow.outputs.parameters.XXXX}} and in Xflow.status.outputs.parameters
...
type Artifact struct {
	...
	// '{{Xflow.outputs.artifacts.XXXX}} and in Xflow.status.outputs.artifacts
	GlobalName string `json:"globalName,omitempty"`
...
// of a single Xflow step, which the executor will use as a default location to store its files.
...
// XflowStep is a reference to a template to execute in a series of step
type XflowStep struct {
	// Name of the step
...
// Item expands a single Xflow step into multiple parallel steps
// The value of Item can be a map, string, bool, or number
type Item struct {
	Value interface{} `json:"value,omitempty"`
}

// Sequence expands a Xflow step into numeric range
...
type Arguments struct {
	// Parameters is the list of parameters to pass to the template or Xflow
	Parameters []Parameter `json:"parameters,omitempty"`

	// Artifacts is the list of artifacts to pass to the template or Xflow
	Artifacts []Artifact `json:"artifacts,omitempty"`
}
...
// XflowStatus contains overall status information about a Xflow
// +k8s:openapi-gen=false
type XflowStatus struct {
	// Phase a simple, high-level summary of where the Xflow is in its lifecycle.
	Phase NodePhase `json:"phase,omitempty"`

	// Time at which this Xflow started
	StartedAt metav1.Time `json:"startedAt,omitempty"`

	// Time at which this Xflow completed
	FinishedAt metav1.Time `json:"finishedAt,omitempty"`

	// A human readable message indicating details about why the Xflow is in this condition.
	Message string `json:"message,omitempty"`

	// Nodes is a mapping between a node ID and the node's status.
	Nodes map[string]NodeStatus `json:"nodes,omitempty"`

	// PersistentVolumeClaims tracks all PVCs that were created as part of the Xflow.
	// The contents of this list are drained at the end of the Xflow.
	PersistentVolumeClaims []apiv1.Volume `json:"persistentVolumeClaims,omitempty"`

	// Outputs captures output values and artifact locations produced by the Xflow via global outputs
	Outputs *Outputs `json:"outputs,omitempty"`
}

// RetryStrategy provides controls on how to retry a Xflow step
type RetryStrategy struct {
	// Limit is the maximum number of attempts when retrying a container
	Limit *int32 `json:"limit,omitempty"`
}

// NodeStatus contains status information about an individual node in the Xflow
// +k8s:openapi-gen=false
...
// Remove returns whether or not the Xflow has completed execution
func (ws *XflowStatus) Completed() bool {
...
// SuspendTemplate is a template subtype to suspend a Xflow at a predetermined point in time
...
func (wf *Xflow) GetTemplate(name string) *Template {
...
func (wf *Xflow) NodeID(name string) string {
